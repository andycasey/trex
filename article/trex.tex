...
Here we self-calibrate the noise in astrometric and radial velocity measurements in the second \Gaia\ data release, and we use excess noise to detect and partially characterise binary systems.
...
After simulating what \Gaia\ would observe and accounting for systematics and detection efficiencies, we find that \todo{$X^{+Y}_{-Z}$}\% \Gaia\ sources have one or more unresolved stellar companions. 
We find the frequency of stellar multiples increases at low metallicities, consistent with \todo{high redshift stuff.} 
For each source we provide probability estimates of that source being a single star, and estimates of orbital elements for systems that are likely to be stellar multiples.
...
stellar multiplicity in different ways. In the next sub-section we 
...
main-sequence binaries present in the \Gaia\ data \citep{Lindegren:2018}.
...
Two equal-sized samples are shown: 
	stars that are suspected to be single after long-term radial velocity monitoring \citep[gray;][]{Soubiran:2013}, and 
	well-studied spectroscopic binaries \citep[black;][]{Pourbaix:2004}.
The samples are equally sized because we required each single star to have a unique spectroscopic binary
...
it is clear that the astrometric and photometric jitter (Eqs. \ref{eq:define-ast-jitter},\ref{eq:define-phot-jitter}) are informative for identifying stellar multiples. While a low astrometric or photometric jitter does
...
\todo{we do not make use of photometric jitter to detect binaries in this work}. 
...
proper motion.\footnote{The photocenter does not change for binary systems
of equal mass and luminosities.} 
The uncertainty in astrometric position induced by orbital motion remains after accounting for the parallax and proper motion. 
...
stellar-mass companion. We take the reduced unit weight error (RUWE) as the measure of
astrometric jitter
\begin{eqnarray}
...
The latter component will contain binary systems, trinaries, and other
...
function of colour and magnitude. We cannot \emph{a priori} write down a 
...
Here we describe that model. Consider a \Gaia\ source of interest. For this source we randomly draw at least 128 
...
distribution has sufficient flexibility to fit the entire distribution of astrometric jitter, so to
...
$s$-subscript here refers to single stars, and $m$ for multiple. Fixing $\mu_m$ as per 
Equation \ref{eq:mu_m} forces 
the \emph{mode} of the log-normal distribution to be at $\mu_s + 3\sigma_s$, ensuring
that the normal distribution is capturing the astrometric jitter of single stars
and the log-normal is capturing the extended tail of stellar multiples. 
We experimented with different priors
...
We optimize the model parameters by minimizing the negative log likelihood
using the \texttt{L-BFGS-B} optimization algorithm \citep{Broyden:1970,Fletcher:1970,Goldfarb:1970,Shanno:1970,Nocedal:2006}. We repeated this procedure for different \Gaia\
sources of interest, where each time this procedure provides us with a
...
we repeat the procedure described above for 1,000 randomly selected sources
...
between data points instead of directly modelling the data \citep[see][for a thorough introduction]{Rasmussen:2005}. This can provide a very flexible generative model for the data with only 
...
%The $\vec{X}$ matrix here has shape $10000 \times 3$: the \bp\ - \rp\ colour, apparent G band magnitude, and absolute G-band magnitude for the 
%10,000 randomly selected points of interest. 
We allow for different scale lengths $\vec{r}$ in
each dimension (i.e. \bp\ - \rp\ colour, apparent G magnitude, absolute G magnitude), and we construct four Gaussian process models: one for each parameter in $\vec\theta$.
...
\todo{With these predictions of astrometric jitter we report the log likelihood of the
...
median radial velocity and estimated error is available for 7,224,631 sources 
\citep{Cropper:2018}.
...
those measurements. 
This allows us to calculate the standard deviation of the single-transit radial velocities,
which we will refer to here as the \emph{radial velocity jitter} $\RVJitter$,
\begin{eqnarray}
...
We assume that the radial velocity jitter for a single star will change depending on the
...
${\teff \approx 5000\,\textrm{K}}$, all else being equal. Stars with bluer colours have fewer absorption lines with deeper wings, 
...
For these reasons, in order to evaluate whether a source is more likely to be a single star or a stellar multiple, we must consider 
the jitter in context with other sources that have a similar properties (\bp\ - \rp\ colour,
apparent \rp\ magnitude, and absolute \rp\ magnitude).
...
and fainter stars have on average lower signal-to-noise (S/N) ratios, both of which result in noisier radial
velocity measurements. The absolute magnitude is similarly important, as giant stars have narrow absorption lines than main-sequence stars of the same temperature or colour. When the source is an unresolved spectroscopic binary (SB2) the radial velocity
...
In \todo{where} we define our estimate of the radial velocity
semi-amplitude and its error (\todo{equations \ref{eq:X} and \ref{eq:Y}}) -- here we simply introduce that significant excess radial velocity jitter is
informative about the radial velocity semi-amplitude.

\todo{
\begin{itemize}
	\item Define $K$ and $K_est$
\end{itemize}
}
...
and so the source radial velocity was removed \citep[see][]{Cropper:2018}. 
...
\todo{
\begin{itemize}
\item Using RP magnitude because RVS
\item Log Likelihood same same.
\item Priors differ?
\item $\tau_rv$
\item Estimates of uncertainty on $\tau$
\end{itemize}
}

\begin{figure*}
    \caption{\todo{Example mixture model.}}
    \label{fig:example-mixture-model}
\end{figure*}
...
We estimated $p(\textrm{single}|\textrm{data})$ for all sources.


\subsection{Simulations} \label{sec:methods/simulations}

The previous sub-sections describe how we define an estimator for whether a source is likely to
be a binary. If that estimator suggests a source is not a binary (e.g., $p(\textrm{single}|\AstJitter, \RVJitter) \approx 1$),
that does not necessarily mean it is a single star because we are not sensitive to detecting
all kinds of binary systems. Here we attempt to estimate that sensitivity as a function
