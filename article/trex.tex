...
\shorttitle{Stellar multiplicity in \Gaia}
...
Here we develop a model to self-calibrate the noise in astrometry, radial velocity, and photometry
in the second \Gaia\ data release.
...
%\end{eqnarray}
\noindent{}and the point estimate of the probability of being a single star given the astrometric jitter $p(\textrm{single}|\AstJitter)$:
\begin{eqnarray}
	p(\textrm{single}|\AstJitter) = \frac{p_{s,a}(\AstJitter\given\hat{\vec\theta})}{p_{s,a}(\AstJitter\given\hat{\vec\theta}) + p_{m,a}(\AstJitter\given\hat{\vec\theta})}
\end{eqnarray}
With these point estimates one can select sources that are likely to be stellar
multiples given their astrometric jitter by requiring $p(\textrm{single}|\AstJitter) \approx 0$.
%and $\lambda_a \gg 1$.
The heuristic for considering what is a likely astrometric binary
...
apparent magnitude, and absolute magnitude). When estimating astrometric jitter we considered that
sources should be similar in the \Gaia\ G band because the G band photometry is used to measure the
photocenter (and astrometric position). However, when considering similarity in magnitudes for radial velocity
jitter we require sources to be similar in the \Gaia\ \rp\ band because this has a central wavelength
closer to that of the radial velocity spectrometer on \Gaia.


When the source is an unresolved spectroscopic binary (SB2) the radial velocity
jitter is a contribution of the expected single-star jitter for the more luminous star
and the contribution of the orbital motion of the system.

% HERE is where we should be considering exactly everything that is going into the gaussian process and our estimates o f K
\todo{Everything we did for the GP}




Consider an edge-on binary system on a circular orbit. Given noiseless estimates of the radial velocity
over one orbital period, the standard deviation of measurements taken at uniformly random times is
related to the radial velocity semi-amplitude of the binary system (Appendix~\ref{app:prove-K}) by:
\begin{eqnarray}
	K \approx \RVJitter\sqrt{2}
\end{eqnarray}

\todo{What about noisy estimates? What about non-ciruclar orbits? Longer periods? Non-uniformity in observation times? Initial phase?}.
\todo{The exact definition we will use for our estimate of the radial velocity semi-amplitude of the system.}
