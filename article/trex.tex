...
\todo{Astrometric binary confirmation: Makarov and Kaplan sample we find 50\% to be binaries, and
we would estimate to be able to find about 45\% binaries. Could re-do this using the parallax for each system, but the result won't change much.}

\begin{figure}
	\centering
	\includegraphics{binary_fraction_wrt_bp_rp.pdf}
	\caption{Mean estimated binary fraction as a function of $\Gbp - \Grp$ colour, which acts as a
			 proxy for stellar type/mass.
			 Ignoring detection efficiency effects, the jitter in astrometry and radial velocity 
			 both suggest an increasing binary fraction with stellar type (or primary mass).}
\end{figure}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{binary_fraction_wrt_metallicity.pdf}
	\caption{Mean estimate binary fraction as a function of stellar metallicity from \project{GALAH},
			 \project{RAVE-on}, and \project{APOGEE} DR16 (left to right).
			 From radial velocity jitter, all cross-matched surveys show an increasing binary fraction 
			 at lower metallicities, which appears to plateau at [Fe/H] $\lesssim -0.75$.
			 The decreasing binary fraction from astrometric jitter is likely a consequence of
			 metal-poor (giant) stars being more distant, making inferences from astrometric jitter
			 more challenging.}
\end{figure}
...
\begin{figure}
	\centering
	\includegraphics{K-vs-MG-with-estimated-Kmax-limits.pdf}
	\caption{Logarithmic-density histogram of the estimated radial velocity semi-amplitude $K$
			 versus the absolute magnitude $\MG$ of each source.
			 The colours indicate the theoretical maximum allowed $K$ value for an equal-mass
			 binary system before experiencing Roche-lobe overflow.}
\end{figure}


\todo{Spectroscopic binary confirmation}
...
\todo{Kmax versus logg}


\subsection{The stellar multiplicity fraction}

\todo{Binary fraction as a function of stellar type (colour/effective temperature/mass)}

\todo{Binary fraction as a function of metallicity}
...
% TODO: Note this is neglecting errors on individual measurements, and biases because \gamma \neq \bar{x}.
